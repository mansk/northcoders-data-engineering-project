...
from src.processing_lambda.jsonl_to_df import get_df_from_s3_bucket, get_latest_df_from_s3_bucket
...
def ingestion_bucket(s3_client):
    s3_client.create_bucket(Bucket="de-watershed-ingestion-bucket", 
            CreateBucketConfiguration={
                'LocationConstraint': 'eu-west-2'
            })
    s3_client.put_object(Bucket="de-watershed-ingestion-bucket", Key="test/file1.jsonl", Body=b'{"key": "value1"}\n{"key": "value2"}')
    s3_client.put_object(Bucket="de-watershed-ingestion-bucket", Key="test/file2.jsonl", Body=b'{"key": "value3"}\n{"key": "value4"}')
    s3_client.put_object(Bucket="de-watershed-ingestion-bucket", Key="table/key_parameter.jsonl", Body=b'{"key": "value1"}\n{"key": "value2"}\n{"key": "value3"}')
...
def test_json_to_latest_df_raises_error_when_invalid_key_given(s3_client, ingestion_bucket):
