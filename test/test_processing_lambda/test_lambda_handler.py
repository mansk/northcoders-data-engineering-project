...
def test_function_reads_table_data_and_converts_to_parquet(s3_client, s3_bucket, s3_processed_bucket, add_bucket_object, event):
...
def test_read_object_from_s3_error_handling(mock_read_object, s3_client, s3_bucket, event, caplog):
...
def test_write_object_to_s3_error_handling(mock_write_object, s3_client, s3_bucket, s3_processed_bucket, event, caplog):
